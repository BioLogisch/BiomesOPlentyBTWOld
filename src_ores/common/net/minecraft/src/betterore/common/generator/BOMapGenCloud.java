...
import net.minecraft.src.betterore.common.generator.tools.BONoiseGenerator;
...
public class BOMapGenCloud extends BOMapGenOreDistribution
{
    @DistributionSetting(
            name = "CloudRadius",
            info = "Cloud horizontal radius, in meters"
    )
    public final BOPDist clRadius = new BOPDist(25.0F, 10.0F);
    @DistributionSetting(
            name = "CloudThickness",
            info = "Cloud thickness (vertical radius), in meters"
    )
    public final BOPDist clThickness = new BOPDist(14.0F, 6.0F);
    @DistributionSetting(
            name = "CloudSizeNoise",
            info = "Noise level added to cloud radius and thickness"
    )
    public final BOPDist clSizeNoise = new BOPDist(0.2F, 0.0F);
    @DistributionSetting(
            name = "CloudHeight",
    ...
    public final BOPDist clInclination;
    @DistributionSetting(
            name = "OreRadiusMult",
    ...
    public final BOPDist orVolumeNoiseCutoff;
    protected static final BODistributionSettingMap _cloudSettingsMap = new BODistributionSettingMap(BOMapGenCloud.class);

    public BOMapGenCloud(int distributionID, boolean canGenerate)
    {
        super(_cloudSettingsMap, distributionID, canGenerate);
        this.clHeight = new BOPDist(32.0F, 16.0F, Type.normal);
        this.clInclination = new BOPDist(0.0F, 0.35F);
        this.orRadiusMult = new BOPDist(1.0F, 0.1F);
        this.orDensity = new BOPDist(0.1F, 0.0F);
        this.orVolumeNoiseCutoff = new BOPDist(0.5F, 0.0F);
    ...
    public Component generateStructure(StructureGroup structureGroup, Random random)
    {
        ...
        {
            BOTransform clMat = new BOTransform();
    ...
    private class DiffuseCloudComponent extends Component
    {
        ...
        protected final BONoiseGenerator noiseGen;
        protected final float sizeNoiseMagnitude;
        protected final int noiseLevels;

        public DiffuseCloudComponent(BOMapGenCloud var1, StructureGroup structureGroup, BOTransform transform, Random random)
        {
            super(structureGroup);
            this.noiseGen = new BONoiseGenerator(random);
