...
public class BOWorldGenSubstitution extends WorldGenerator implements IBOOreDistribution
{
    @DistributionSetting(
            name = "Name",
            inherited = false,
    ...
    public final BOBlockDescriptor oreBlock;
    @DistributionSetting(
    ...
    protected static final BODistributionSettingMap settingMap = new BODistributionSettingMap(BOWorldGenSubstitution.class);

    public BOWorldGenSubstitution(int distributionID, boolean canGenerate)
    {

        this.oreBlock = new BOBlockDescriptor(Integer.toString(Block.stone.blockID));
        this.replaceableBlocks = new BOBlockDescriptor();
    ...
    {
        if (inherits != null && inherits instanceof BOWorldGenSubstitution)
        {
            settingMap.inheritAll((BOWorldGenSubstitution)inherits, this);
            this._valid = false;
        }
        else
        {
                            ...
                            for (int z = minZ; z < maxZ; ++z)
                            {
                                ...
                                if (biome == null || this.biomes.getWeight(biome) > 0.5F)
                                {
                                    for (int y = minh; y <= maxh; ++y)
                                    {
                                    	...
                                    	BOLogger.log.fine("Gen Substituiion:" + this.placedBlocks);
